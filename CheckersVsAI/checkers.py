...
# Always resolve image paths relative to this script's location
...
class piece:
    ...
    def __str__(self):
         if self.colour == "white":
                ...
                return f"{bcolors.FAIL}"+"o"+f"{bcolors.ENDC}"
    def getSprite(self):
        filename = "white.png" if self.colour == "white" else "black.png"
    ...
    def takeMoves(self, coordinate):
        ...
        for i in vectorList:
            ...
            if x < 0 or y < 0 or x > 7 or y > 7:
                ...
                pass
            elif self.pieceAtPos((x, y)).getColour() != piece.getColour():
                lx = x + i[0]
                ly = y + i[1]
                if lx < 0 or ly < 0 or lx > 7 or ly > 7:
                    pass
                elif self.pieceAtPos([lx, ly]) == 0:
                    # path starts at landing, first jumped enemy tracked from the start
                    initialJumps.append(([(lx, ly)], (x, y)))

        output = []
        for path, firstJumped in initialJumps:
            output += self.takeMovesRec(path, [firstJumped], piece)
        return output



    def takeMovesRec(self, path, jumpedPieces, piece):
        xog, yog = path[-1][0], path[-1][1]

        vectorList = piece.getMoves()
        possibleJumps = []
        for i in vectorList:
            ex = xog + i[0]   # enemy piece position
            ey = yog + i[1]
            if ex < 0 or ey < 0 or ex > 7 or ey > 7:
                pass
            elif self.pieceAtPos((ex, ey)) == 0:
                pass
            elif (ex, ey) in jumpedPieces:  # already jumped this piece
                pass
            elif self.pieceAtPos((ex, ey)).getColour() != piece.getColour():
                lx = ex + i[0]  # landing position
                ly = ey + i[1]
                if lx < 0 or ly < 0 or lx > 7 or ly > 7:
                    pass
                elif self.pieceAtPos([lx, ly]) == 0:
                    possibleJumps.append(((lx, ly), (ex, ey)))

        if len(possibleJumps) == 0:
            return [path]
        else:
            output = []
            for landing, jumped in possibleJumps:
                newJumped = jumpedPieces + [jumped]  # mark this enemy as jumped
                output += self.takeMovesRec(path + [landing], newJumped, piece)
            return output
...
#Add promoting piece counter increment 
    ...
    def isWon(self, colour: str) -> bool:
        if self._currentboard.points[1] == 0 and colour == "white":
            return True
        elif self._currentboard.points[0] == 0 and colour == "black":
            return True
        else:
            return False
...
class AI:
    ...
    def miniMaxAlphaBeta(self, game, depth, player, alpha, beta):
        if depth == 0:
            return self.staticEval(game), []

        allAvailableMoves = game.allPosMoves(player)
        bestMove = []
            ...
            for posPieceMoves in allAvailableMoves:
                for posMove in posPieceMoves:
                    if posMove != []:
                        copyOfGame = copy.deepcopy(game)
                        copyOfGame.move(posMove[0], posMove[-1])
                        if copyOfGame.isWon(self.colour):
                            return 100000, posMove
                        if copyOfGame.isWon(self.OppColour):
                            return -100000, posMove
                        newPlayer = "black" if player == "white" else "white"
                        value, _ = self.miniMaxAlphaBeta(copyOfGame, depth-1, newPlayer, alpha, beta)
                        if value > bestVal:
                            bestVal = value
                            bestMove = posMove
...
class GameScreen():
        ...
        for i in range(8):
            for j in range(8):
                ...
                if currentPiece != 0:
                    ...
                    bg = "black" if (i+j)%2 == 1 else "white"
                    label = tk.Button(master=frame, image=sprite, background=bg, activebackground=bg,
                                      borderwidth=0, height=40, width=50,
                                      command=lambda c=i*8+j: self.clickOnPiece(currentBoard.possiblePieceMoves((c%8, c//8)), 63-(c%8 + (c//8)*8)))
                    label.image = sprite  # keep reference to prevent garbage collection
    ...
    def movePiece(self, coordinate1, coordinate2):
        ...
        # Apply the player's move immediately and redraw so it's visible
        self._game.playerMove((i1, j1), (i2, j2))
        self.updateBoard()

        if self._game.isWon("white"):
            WinningMenu().WinScreen()
            return

        # Run the AI move in the background so the UI stays responsive
        def run():
            self._game.aiMove()
            self._board.after(0, self._check_after_ai)

        threading.Thread(target=run, daemon=True).start()

    def _check_after_ai(self):
        self.updateBoard()
        if self._game.isWon("black"):
            LoseMenu().LoseScreen()
...
class AIGame(game):
    ...
    def playerMove(self, pieceCoordinate, moveToCoordinate):
        super().move(pieceCoordinate, moveToCoordinate)

    def aiMove(self):
        if super().isWon("white"):
            return
        if self._AI.difficulty == 1:
            depth = 3
        elif self._AI.difficulty == 2:
            depth = 5
        else:
            depth = 7
        _, move = self._AI.miniMaxAlphaBeta(self, depth, "black", -10000000, 10000000)
        if move != []:
...
class GameMenu:
    ...
    def openStartMenu(self): #Opens the Start menu window
        ...
        def startGame():
            ...
            for widget in start_menu.winfo_children():
                widget.destroy()
            start_menu.title('Checkers')
            board = ttk.Frame(start_menu)
            board.pack()
...
class WinningMenu:
    ...
    def WinScreen(self):
        ...
        win_screen.geometry('1050x700')
        font_style = font.Font(family='Arial', size=15, weight='bold')

        def CloseGame():
            win_screen.destroy()

        tk.Label(master=win_screen, text='Congratulations, You won!', fg='black', bg='white',
                 font=font_style, height=5, width=30).place(x=340, y=100)
        tk.Button(master=win_screen, text='Close Game', fg='black', bg='white',
                  font=font_style, height=5, width=20, command=CloseGame).place(x=400, y=400)
...
class DrawMenu:
    def __init__(self):
        pass
    def DrawScreen(self):
        draw_screen = Toplevel()
        draw_screen.title('You Drew')
        draw_screen.geometry('1050x700')
        font_style = font.Font(family='Arial', size=15, weight='bold')

        def CloseGame():
            draw_screen.destroy()

        tk.Label(master=draw_screen, text='You drew!', fg='black', bg='white',
                 font=font_style, height=5, width=30).place(x=340, y=100)
        tk.Button(master=draw_screen, text='Close Game', fg='black', bg='white',
                  font=font_style, height=5, width=20, command=CloseGame).place(x=400, y=400)
...
class LoseMenu:
    def __init__(self):
        pass
    def LoseScreen(self):
        lose_screen = Toplevel()
        lose_screen.title('You lost!')
        lose_screen.geometry('1050x700')
        font_style = font.Font(family='Arial', size=15, weight='bold')

        def CloseGame():
            lose_screen.destroy()

        tk.Label(master=lose_screen, text='Unlucky, You Lost!', fg='black', bg='white',
                 font=font_style, height=5, width=30).place(x=340, y=100)
        tk.Button(master=lose_screen, text='Close Game', fg='black', bg='white',
                  font=font_style, height=5, width=20, command=CloseGame).place(x=400, y=400)
